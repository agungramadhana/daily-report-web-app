...
var report = function () {
    ...
    var maps = function (lat, long, name) {
        ...
        var latitude = lat == null ? $('#latitude').val().replace(',', '.') : lat
        var longitude = long == null ? $('#longitude').val().replace(',', '.') : long
        var areaName = name == null ? $('#area-name').val() : name
        ...
        // Buat map baru hanya jika container ada
        if ($('#kt_leaflet_3').length) {
            currentMap = L.map('kt_leaflet_3', {
                center: [-8.85, 125.60], // default center on Dili
                zoom: 8
            });
            ...
            // Coba tambahkan marker jika ada latitude dan longitude
            try {
                if (latitude && longitude) {
                    var marker = L.marker([latitude, longitude], { icon: leafletIcon }).addTo(currentMap);
                    if (areaName) {
                        marker.bindPopup(areaName, { closeButton: false });
                    }
                    // Pusatkan map ke marker
                    currentMap.setView([latitude, longitude], 8);
                }
            } catch (e) {
                console.error("Gagal menambahkan marker:", e);
            }
